...
frequency_tbl = {0: 22050, 1: 24000, 2: 16000, 3: 44100, 4: 48000, 5:
                 32000, 6: 64000}
...
def get_l4(s):
    return reduce(lambda a, b: ((a << 8) + b), map(int, s))
...
def get_head(word):
    if len(word) != 4:
        return {}
    l = word[0] << 24 | word[1] << 16 | word[2] << 8 | word[3]
...
def mp3info(fn):
    ...
    if xh:
        ...
        if vbr:
            ...
            vbr_average = (xing_bytes * 8.) / (tpf * xing_frames * 1000)
    ...
    if h['freq_idx'] >= 0:
        ...
        i['FREQUENCY'] = ''
    i['VBR'] = bool(vbr)
    i['VBR_AVERAGE'] = vbr_average
    i['LENGTH'] = int(h['length'] / 100)
...
def get_mp3tag(fn):
    ...
    if line[:3] == 'TAG':
        ...
        i = 0
        j = i + 3
        #id['d1'] = line[i:j].strip()
        i = j
        j = i + 30
        id['TITLE'] = line[i:j].strip()
        i = j
        j = i + 30
        id['ARTIST'] = line[i:j].strip()
        i = j
        j = i + 30
        id['ALBUM'] = line[i:j].strip()
        i = j
        j = i + 4
        id['YEAR'] = line[i:j].strip()
        i = j
        j = i + 28
        id['COMMENT'] = line[i:j].strip()
...
def Categorize(fn):
    ...
    path1 = "cats/GENRE_ARIST/%s/%s" % (
        i2.get('GENRE', "Unknown"), i2.get('ARTIST', "Unknown"))
    path2 = "cats/GENRE/%s" % (i2.get('GENRE', "Unknown"), )
    path3 = "cats/ARIST/%s" % (i2.get('ARTIST', "Unknown"), )

    path1 = path1.replace("\0", "_")
    path1 = path1.replace(" ", "_")
    path2 = path2.replace("\0", "_")
    path2 = path2.replace(" ", "_")
    path3 = path3.replace("\0", "_")
